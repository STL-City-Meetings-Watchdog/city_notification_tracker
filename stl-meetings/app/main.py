...
def init_db():
    ...
    # --- NEW TABLE: stores full text of every scraped HTML page ---
...
def extract_text_from_html(soup):
    """Return clean plain text from a BeautifulSoup object."""
    for tag in soup(['script', 'style', 'nav', 'footer', 'header']):
        tag.decompose()
    return ' '.join(soup.get_text(separator=' ').split())

def extract_text_from_pdf(pdf_bytes):
    """Return plain text from raw PDF bytes using pdfminer.six."""
    try:
...
def save_scraped_page(conn, meeting_id, url, page_type, soup):
    """Extract text from a parsed page and store it in scraped_pages."""
    try:
        text = extract_text_from_html(soup)
        if text:
            conn.execute(
                'INSERT OR REPLACE INTO scraped_pages (meeting_id, url, page_type, page_text) VALUES (?,?,?,?)',
                (meeting_id, url, page_type, text)
            )
            logger.info(f"Saved scraped page text ({len(text)} chars) for {url}")
    except Exception as e:
        logger.error(f"Failed to save scraped page {url}: {e}")

# ---------------------------------------------------------------------------
# Scraping (now accepts conn + meeting_id to persist page text)
# ---------------------------------------------------------------------------
...
def scrape_event_page(event_url, conn=None, meeting_id=None):
    documents = []
    try:
        ...
        # Persist the event page text
        if conn and meeting_id:
            save_scraped_page(conn, meeting_id, event_url, 'event', soup)

        for link in soup.find_all('a', href=True):
            href = link['href']
            text = link.get_text().lower()
            if any(x in text for x in ['agenda', 'meeting materials', 'packet', 'minutes', 'board bill']) \
                    or '/documents/' in href or '/board-bills/' in href:
                ...
                if href.endswith('.pdf'):
                    ...
                    documents.extend(scrape_documents_page(full_url, conn=conn, meeting_id=meeting_id))
                elif '/board-bills/' in href:
                    documents.extend(scrape_board_bill_page(full_url, conn=conn, meeting_id=meeting_id))
    except Exception as e:
        logger.error(f"Failed to scrape {event_url}: {e}")
    return documents

def scrape_documents_page(doc_url, conn=None, meeting_id=None):
    documents = []
    try:
        ...
        if conn and meeting_id:
            save_scraped_page(conn, meeting_id, doc_url, 'documents', soup)
...
def scrape_board_bill_page(bill_url, conn=None, meeting_id=None):
    documents = []
    try:
        ...
        if conn and meeting_id:
            save_scraped_page(conn, meeting_id, bill_url, 'board_bill', soup)
...
def download_document(url, meeting_id, doc_type):
    """Download a document; extract and return text if it's a PDF."""
    try:
        resp = proxy_get(url, timeout=60)
        resp.raise_for_status()
        ...
        return str(local_path.relative_to(PDF_DIR)), filename, extracted_text
    except Exception as e:
...
def check_upcoming_documents():
    ...
    for m in meetings:
        ...
        docs = scrape_event_page(url, conn=conn, meeting_id=mid)
        for doc in docs:
            ...
            if local_path:
                c.execute(
                    ...
                    (mid, doc["type"], doc["url"], local_path, filename, extracted_text)
                )
...
def sync_meetings():
    ...
    for event in events:
        ...
        existing = c.fetchone()
        if existing:
            c.execute('UPDATE meetings SET title=?,description=?,location=?,start_time=?,end_time=?,event_url=?,sponsor=?,contact_name=?,contact_email=?,contact_phone=?,updated_at=CURRENT_TIMESTAMP WHERE uid=?',
                (data['title'],data['description'],data['location'],data['start_time'],data['end_time'],data['event_url'],data['sponsor'],data['contact_name'],data['contact_email'],data['contact_phone'],data['uid']))
        else:
            c.execute('INSERT INTO meetings (uid,title,description,location,start_time,end_time,event_url,sponsor,contact_name,contact_email,contact_phone) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
                (data['uid'],data['title'],data['description'],data['location'],data['start_time'],data['end_time'],data['event_url'],data['sponsor'],data['contact_name'],data['contact_email'],data['contact_phone']))
    ...
    for meeting in new_meetings:
        ...
        if meeting['event_url']:
            for doc in scrape_event_page(meeting['event_url'], conn=conn, meeting_id=meeting['id']):
...
def search():
    ...
    for r in c.fetchall():
        results.setdefault(r['id'], dict(r))
    conn.close()
    return render_template('search.html', results=list(results.values()), query=q)
